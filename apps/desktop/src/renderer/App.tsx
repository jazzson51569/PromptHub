...
function App() {
  ...
  const indentPrompt = usePromptStore((state) => state.indentPrompt);
  const outdentPrompt = usePromptStore((state) => state.outdentPrompt);
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      // Handle Tab/Shift+Tab for prompt indentation/outdentation
      // 处理 Tab/Shift+Tab 用于提示词层级缩进/取消缩进
      if (e.key === "Tab" && selectedIds.length > 0) {
        // Check if we're in an input/textarea - don't interfere with text editing
        // 检查是否在输入框/文本域中 - 不干扰文本编辑
        const target = e.target as HTMLElement;
        const isInput = target.tagName === "INPUT" || 
                        target.tagName === "TEXTAREA" || 
                        target.isContentEditable;
        
        console.log(`[TabHandler] Tab key pressed, selectedIds: ${JSON.stringify(selectedIds)}, isInput: ${isInput}, shiftKey: ${e.shiftKey}, target tag: ${target.tagName}`);
        
        if (!isInput) {
          e.preventDefault();
          if (e.shiftKey) {
            // Shift+Tab: outdent (promote to parent level)
            // Shift+Tab: 取消缩进（提升到父层级）
            console.log(`[TabHandler] Executing outdent for prompts: ${selectedIds.join(', ')}`);
            for (const id of selectedIds) {
              void outdentPrompt(id).then(() => {
                console.log(`[TabHandler] Outdent completed for prompt: ${id}`);
              }).catch((error) => {
                console.error(`[TabHandler] Outdent failed for prompt: ${id}`, error);
              });
            }
          } else {
            // Tab: indent (make child of previous sibling)
            // Tab: 缩进（成为前一个同级的子项）
            console.log(`[TabHandler] Executing indent for prompts: ${selectedIds.join(', ')}`);
            for (const id of selectedIds) {
              void indentPrompt(id).then(() => {
                console.log(`[TabHandler] Indent completed for prompt: ${id}`);
              }).catch((error) => {
                console.error(`[TabHandler] Indent failed for prompt: ${id}`, error);
              });
            }
          }
          return;
        } else {
          console.log(`[TabHandler] Skipping Tab handling - focused on input element: ${target.tagName}`);
        }
      }
  ...
  }, [shortcutModes, localShortcuts, selectedIds, indentPrompt, outdentPrompt]);
