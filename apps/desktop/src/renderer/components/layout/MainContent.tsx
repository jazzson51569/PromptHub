...
import { PromptListView } from '../prompt/PromptListView';
...
const FileDetailPanel = lazy(() => import('../prompt/FileDetailPanel').then(m => ({ default: m.FileDetailPanel })));
...
function PromptSkillMainContent() {
  ...
  // Handle moving prompt to a new parent (hierarchical)
  // 处理将提示词移动到新的父节点（层级结构）
  const handleMovePromptToParent = useCallback(async (promptId: string, newParentId: string | null, newOrder: number) => {
  ...
  // 文件详情面板状态
  const [isFileDetailPanelOpen, setIsFileDetailPanelOpen] = useState(false);
  ...
  // View details - show modal or panel
  // 查看详情 - 弹窗或面板显示
  ...
  // 在侧边面板中查看详情
  const handleViewDetailPanel = (prompt: Prompt) => {
    setDetailPrompt(prompt);
    setIsFileDetailPanelOpen(true);
  };
  ...
  const menuItems: ContextMenuItem[] = useMemo(() => {
    ...
    return [
      ...
      {
        label: t('prompt.viewDetailPanel', '查看详情面板'),
        icon: <CheckIcon className="w-4 h-4" />,
        onClick: () => handleViewDetailPanel(contextMenu.prompt),
      },
      {
        label: t('prompt.edit'),
  ...
  return (
    <main className="flex-1 relative overflow-hidden app-wallpaper-section">
        ...
        >
          ...
          {sortedPrompts.length === 0 ? (
            ...
            <PromptListView
              ...
              onCopy={handleCopyPrompt}
              onContextMenu={handleContextMenu}
              onMovePrompt={handleMovePromptToParent}
              sortBy={sortBy}
              sortOrder={sortOrder}
            />
          )}
      ...
      {/* File detail panel */}
      {/* 文件详情面板 */}
      {isFileDetailPanelOpen && (
        <Suspense fallback={null}>
          <FileDetailPanel
            isOpen={isFileDetailPanelOpen}
            onClose={() => {
              setIsFileDetailPanelOpen(false);
              setDetailPrompt(null);
            }}
            prompt={detailPrompt}
            onEdit={(prompt) => setEditingPrompt(prompt)}
            onTest={(prompt) => handleAiTestFromTable(prompt, 'single')}
          />
        </Suspense>
      )}
