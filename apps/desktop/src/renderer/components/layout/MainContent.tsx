      ...
      className={`
        ...
        ${isSelected
          ...
          : isDropTarget && dropPosition === 'inside'
            ...
            : depth > 0
              ? 'prompt-list-card bg-green-100 hover:bg-green-200'
...
interface VirtualizedPromptListProps {
  ...
  onToggleCollapse: (promptId: string) => void;
}
...
const VirtualizedPromptList = memo(function VirtualizedPromptList({
  ...
  useEffect(() => {
    ...
    const staleIds = [...collapsedPromptIds].filter((promptId) => !promptIds.has(promptId));
    if (staleIds.length > 0) {
      const next = new Set([...collapsedPromptIds].filter((promptId) => promptIds.has(promptId)));
      onToggleCollapse(staleIds[0]);
    }
  }, [prompts, collapsedPromptIds, onToggleCollapse]);
          ...
          return (
            ...
            >
              <PromptCard
                ...
                onToggleCollapse={() => onToggleCollapse(prompt.id)}
...
function PromptSkillMainContent() {
  ...
  const handleDoubleClickPrompt = useCallback((prompt: Prompt, _e: React.MouseEvent) => {
    selectPrompt(prompt.id);
    setTimeout(() => {
      openDetailInlineEdit('title');
    }, 50);
  }, [selectPrompt, openDetailInlineEdit]);
  ...
  const handleMoveToPrompt = useCallback(async (sourcePrompt: Prompt, targetPromptId: string) => {
    try {
      const targetPrompt = prompts.find((p) => p.id === targetPromptId);
      const hierarchyMeta = getPromptHierarchyMeta(prompts);
      const childCount = hierarchyMeta.childCountById.get(targetPromptId) ?? 0;
      await movePrompt(sourcePrompt.id, targetPromptId, childCount);
      if (targetPrompt) {
        showToast(`${t('toast.movedToPrompt') || '已移动到'}「${targetPrompt.title}」`, 'success');
      } else {
        showToast(t('toast.movedToPrompt') || '已移动', 'success');
      }
    } catch (error) {
      console.error('Failed to move prompt to node:', error);
      showToast(t('prompt.moveToPromptFailed', 'Failed to move prompt'), 'error');
    }
  }, [prompts, movePrompt, showToast, t]);
  ...
  const menuItems: ContextMenuItem[] = useMemo(() => {
    ...
    const hierarchyMeta = getPromptHierarchyMeta(prompts);
    const moveToNodeItems: ContextMenuItem[] = prompts
      .filter((p) => {
        const childCount = hierarchyMeta.childCountById.get(p.id) ?? 0;
        return childCount > 0 && p.id !== contextMenu.prompt.id;
      })
      .map((prompt) => ({
        label: prompt.title,
        description: `${hierarchyMeta.childCountById.get(prompt.id) ?? 0} ${t('prompt.children', 'children')}`,
        icon: <GitBranchIcon className="w-4 h-4" />,
        onClick: () => void handleMoveToPrompt(contextMenu.prompt, prompt.id),
      }));
    ...
    return [
      ...
      {
        label: t('prompt.collapseAllPrompts'),
  ...
  }, [contextMenu, flattenedFolders, folderPathById, t, toggleFavorite, togglePinned, handleViewDetail, handleCopyPrompt, handleDuplicatePrompt, handleSharePrompt, handleAiTestFromTable, handleVersionHistory, handleDeletePrompt, handleMovePrompt, prompts, handleMoveToPrompt]);
        ...
        >
          ...
          {sortedPrompts.length === 0 ? (
            ...
            <VirtualizedPromptList
              ...
              onDoubleClick={handleDoubleClickPrompt}
              onContextMenu={handleContextMenu}
              onMovePrompt={handleMovePromptInTree}
              collapsedPromptIds={collapsedPromptIds}
              onToggleCollapse={togglePromptCollapse}
            />
          )}
