...
import { CopyIcon, CheckIcon, BracesIcon, HistoryIcon, CalendarIcon, ClockIcon, PlayIcon, Loader2Icon, ImageIcon, XIcon, Edit2Icon } from 'lucide-react';
import type { ChatImageAttachment } from '../../services/ai';
import { parsePromptVariables,
...
export function VariableInputModal({
  ...
  const [variableNameMap, setVariableNameMap] = useState<Record<string, string>>({});
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      parsedVariables.forEach((v) => {
        initialVars[v.name] = history[v.name] || v.defaultValue || v.name;
        initialNameMap[v.name] = v.name;
      });

      setVariables(initialVars);
      setVariableNameMap(initialNameMap);
      setCopied(false);
      setImageAttachments([]);
    }
  ...
  const handleStartRename = (variableName: string) => {
    setEditingVariableName(variableName);
    setTempVariableName(variableNameMap[variableName] || variableName);
  };

  const handleConfirmRename = async (originalName: string) => {
    ...
    setVariableNameMap((prev) => ({ ...prev, [originalName]: newName }));

    try {
      const regex = new RegExp(`\\{\\{\\s*${originalName}\\s*(?::[^}]*)?\\s*\\}\\}`, 'g');
      const updatedUserPrompt = userPrompt.replace(regex, `{{${newName}}}`);
      let updatedSystemPrompt = systemPrompt;
      if (systemPrompt) {
        updatedSystemPrompt = systemPrompt.replace(regex, `{{${newName}}}`);
      }
      ...
      showToast(t('prompt.variableRenamed', '变量已重命名'), 'success');
    } catch (error) {
      console.error('Failed to rename variable:', error);
      showToast(t('prompt.variableRenameFailed', '变量重命名失败'), 'error');
    }
  ...
  const filledPrompt = useMemo(() => {
    ...
    Object.entries(variableNameMap).forEach(([originalName, newName]) => {
      if (originalName !== newName) {
        const regex = new RegExp(`\\{\\{\\s*${originalName}\\s*(?::[^}]*)?\\s*\\}\\}`, 'g');
        replacedResult = replacedResult.replace(regex, `{{${newName}}}`);
      }
    });

    const renamedVariables: Record<string, string> = {};
    Object.entries(variables).forEach(([originalName, value]) => {
      const newName = variableNameMap[originalName] || originalName;
      renamedVariables[newName] = value;
    });

    return replacePromptVariables(replacedResult, renamedVariables);
  }, [mode, systemPrompt, userPrompt, variables, variableNameMap]);
  ...
  const replaceVariables = (text: string) => {
    ...
    result = replacePromptVariables(result, renamedVariables);
              ...
              {parsedVariables.map((v) => (
                <div key={v.name} className="space-y-1.5">
                  <div className="flex items-start justify-between gap-2">
                    <div className="flex items-center gap-2">
                      <label className="text-sm font-medium text-foreground">
                        {t('prompt.variableName', '变量名')}: <span className="text-primary">{variableNameMap[v.name] || v.name}</span>
                      </label>
                      {editingVariableName === v.name ? (
                        <div className="flex items-center gap-1">
                          ...
                          <button
                            type="button"
                            onClick={() => handleConfirmRename(v.name)}
                        ...
                        <button
                          type="button"
                          onClick={() => handleStartRename(v.name)}
                  ...
                  <textarea
                    ...
                    placeholder={`${t('prompt.inputVariable', { name: variableNameMap[v.name] || v.name })}${v.defaultValue ? ` ${t('prompt.exampleHint', '例如')}: ${v.defaultValue}` : ''}`}
                    rows={1}
