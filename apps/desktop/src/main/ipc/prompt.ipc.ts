...
export function registerPromptIPC(db: PromptDB, folderDb: FolderDB, rawDb: Database.Database): void {
  ...
  // Move Prompt (hierarchical operation)
  // 移动提示词（层级操作）
  ipcMain.handle(IPC_CHANNELS.PROMPT_MOVE, async (_, id: string, newParentId: string | null, newOrder: number) => {
    db.movePrompt(id, newParentId, newOrder);
    syncWorkspace();
    return true;
  });
