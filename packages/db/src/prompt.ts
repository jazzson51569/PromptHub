...
export class PromptDB {
   ...
   * 移动提示词到新的父级或在同一父级内重新排序
   */
  movePrompt(id: string, newParentId: string | null, newOrder: number): void {
    const txn = this.db.transaction(() => {
      // Update the moved prompt
      this.db
        .prepare("UPDATE prompts SET parent_id = ?, sort_order = ?, updated_at = ? WHERE id = ?")
        .run(newParentId, newOrder, Date.now(), id);

      // Reorder siblings
      const siblings = this.db
        .prepare("SELECT id FROM prompts WHERE parent_id = ? AND id != ? ORDER BY sort_order")
        .all(newParentId, id) as { id: string }[];

      siblings.forEach((sibling, index) => {
        const adjustedIndex = index >= newOrder ? index + 1 : index;
        this.db
          .prepare("UPDATE prompts SET sort_order = ?, updated_at = ? WHERE id = ?")
          .run(adjustedIndex, Date.now(), sibling.id);
      });
    });
  ...
  /**
   * Reorder prompts within the same parent
   * 在同一父级内重新排序提示词
   */
  reorderPrompts(ids: string[], parentId: string | null): void {
    const txn = this.db.transaction(() => {
      ids.forEach((id, index) => {
        this.db
          .prepare("UPDATE prompts SET sort_order = ?, parent_id = ?, updated_at = ? WHERE id = ?")
          .run(index, parentId, Date.now(), id);
      });
    });
